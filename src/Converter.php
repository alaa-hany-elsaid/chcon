 ...
 * UCS table directory
 */
define('CHCON_UCSTABLE_DIR', realpath(dirname(__FILE__)) . DIRECTORY_SEPARATOR . 'ucstable' . DIRECTORY_SEPARATOR);

/**
 * Replacement Character
 */
define('CHCON_REPLACEMENT_CHARACTER', 0xFFFD);

/**
 * UTF-8 BOM
 */
define('CHCON_UTF8_BOM', "\xEF\xBB\xBF");

/**
 * UTF-16 (Big Endian) BOM
 */
define('CHCON_UTF16BE_BOM', "\xFE\xFF");

/**
 * UTF-16 (Lil Endian) BOM
 */
define('CHCON_UTF16LE_BOM', "\xFF\xFE");

/**
 * UTF-32 (Big Endian) BOM
 */
define('CHCON_UTF32BE_BOM', "\x00\x00\xFE\xFF");

/**
 * UTF-32 (Lil Endian) BOM
 */
define('CHCON_UTF32LE_BOM', "\xFF\xFE\x00\x00");
...
class Converter
{
    ...
    else
    {
      ...
      else
      {
        ...
        foreach ($token as $char)
        {
          ...
          {
            $ucs[] = CHCON_REPLACEMENT_CHARACTER;
          }
        }
      }
      ...
      else
      {
        ...
        foreach ($ucs as $uni)
        {
          if (isset($table[$uni]))
          {
            $result .= Converter::_dec2bin($table[$uni]);
          }
          else
          {
  ...
  {
    $filename = CHCON_UCSTABLE_DIR . $charset . '.map';
  ...
  function _decodeISO2022CN($string)
  {
    ...
    for ($i = 0, $j = strlen($string); $i < $j; ++$i)
    {
      if ($string[$i] == "\x1B")
      {
        ...
        $result[] = CHCON_REPLACEMENT_CHARACTER;
        continue;
      }
      ...
      {
        $n = Converter::_bin2dec($string[$i] . $string[++$i]);
    ...
    for ($i = 0, $j = count($ucs); $i < $j; ++$i)
    {
      foreach ($table as $k => $v)
      {
        if (isset($v[$ucs[$i]]))
        {
          ...
          $result .= Converter::_dec2bin($v[$ucs[$i]]);
  ...
  function _decodeISO2022JP3($string)
  {
    ...
    for ($i = 0, $j = strlen($string); $i < $j; ++$i)
    {
      ...
      elseif ($esc == "\x28\x49")
      {
        $n = Converter::_bin2dec($string[$i]) + 0x80;
  ...
  function _encodeUCS2($ucs)
  {
    ...
    for ($i = 0, $j = count($ucs); $i < $j; ++$i)
    {
      ...
      {
        $ucs[$i--] = CHCON_REPLACEMENT_CHARACTER;
      }
    }
  ...
  function _decodeUTF8($string)
  {
    ...
    if (strpos($string, CHCON_UTF8_BOM) === 0)
    {
  ...
  {
    return CHCON_UTF8_BOM . $this->_encodeUTF8N($ucs);
  }
  ...
  {
    if (strpos($string, CHCON_UTF16BE_BOM) === 0)
    {
      return $this->_decodeUTF16BE(substr($string, 2));
    }
    elseif (strpos($string, CHCON_UTF16LE_BOM) === 0)
    {
  ...
  {
    return CHCON_UTF16BE_BOM . $this->_encodeUTF16BE($ucs);
  }
  ...
  {
    if (strpos($string, CHCON_UTF32BE_BOM) === 0)
    {
      return $this->_decodeUTF32BE(substr($string, 4));
    }
    elseif (strpos($string, CHCON_UTF32LE_BOM) === 0)
    {
  ...
  {
    return CHCON_UTF32BE_BOM . $this->_encodeUTF32BE($ucs);
  }
